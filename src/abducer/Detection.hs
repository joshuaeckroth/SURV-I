...
mergeMultCameraDets [] = []
mergeMultCameraDets (hdet@(Hyp {entity = det}):hdets) = [hdet] ++ (mergeMultCameraDets $
                                              filter (\(Hyp {entity = det'}) -> not $ likelySameDet det det') hdets)

-- ^ Check if two detections are likely the same (possibly across different cameras).
likelySameDet :: Detection -> Detection -> Bool
likelySameDet det1 det2 = (detDist det1 det2 < 20.0) && (detDelta det1 det2 < 1.0)
